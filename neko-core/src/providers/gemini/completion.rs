...
impl completion::CompletionModel for CompletionModel {
    ...
    async fn completion(
        ...
        match response.usage_metadata {
            Some(ref usage) => tracing::info!(target: "neko",
            "Gemini completion token usage: {}",
            usage
            ),
            None => tracing::info!(target: "neko",
...
pub mod gemini_api_types {
    ...
    pub struct ContentCandidate {
        ...
        /// These are passages that are "recited" from copynekohted material in the foundational LLM's training data.
    ...
    /// ### neko Note:
    /// Can be used to cosntruct a typesafe `additional_params` in neko::[AgentBuilder](crate::agent::AgentBuilder).
    #[derive(Debug, Deserialize, Serialize)]
    #[serde(rename_all = "camelCase")]
