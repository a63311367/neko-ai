...
fn lancedb_to_neko_error(e: lancedb::Error) -> VectorStoreError {
    VectorStoreError::DatastoreError(Box::new(e))
}

fn serde_to_neko_error(e: serde_json::Error) -> VectorStoreError {
...
/// # Example
/// ```
/// use neko_lancedb::{LanceDbVectorIndex, SearchParams};
/// use neko::providers::openai::{Client, TEXT_EMBEDDING_ADA_002, EmbeddingModel};
///
...
/// # Example
/// ```
/// let search_params = neko_lancedb::SearchParams::default().distance_type(lancedb::DistanceType::Cosine);
/// ```
...
impl<M: EmbeddingModel + Sync + Send> VectorStoreIndex for LanceDbVectorIndex<M> {
    ...
    /// ```
    /// use neko_lancedb::{LanceDbVectorIndex, SearchParams};
    /// use neko::providers::openai::{EmbeddingModel, Client, TEXT_EMBEDDING_ADA_002};
    ///
    /// let openai_client = Client::from_env();
    ///
        ...
        self.build_query(query)
            ...
            .map(|(i, value)| {
                Ok((
                    ...
                    serde_json::from_value(value).map_err(serde_to_neko_error)?,
                ))
            })
            .collect()
    }
