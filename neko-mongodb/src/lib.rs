...
fn mongodb_to_neko_error(e: mongodb::error::Error) -> VectorStoreError {
...
/// # Example
/// ```rust
/// use neko_mongodb::{MongoDbVectorIndex, SearchParams};
    ...
    for MongoDbVectorIndex<M, C>
{
    ...
    async fn top_n<T: for<'a> Deserialize<'a> + Send>(
        ...
        while let Some(doc) = cursor.next().await {
            let doc = doc.map_err(mongodb_to_neko_error)?;
